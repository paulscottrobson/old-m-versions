...
class KernelImage(object):
	...
	def send(self,byte):
		if False:
			...
			print("{2:04x} {0:02x} {1}".format(byte,chr(c),self.pointer))
		self.binary[self.pointer] = byte
		self.pointer += 1
	#
	def requires(self,count):
		nextPointer = self.pointer + count
		if (nextPointer >> 10) / (self.pointer >> 10):
			nextPointer = nextPointer >> 10
			nextPointer = (nextPointer+1) << 10
