...
DefineNewWord:
		...
		ld 		a,(DefaultCreateBits)				; get any default create bits
		or 		(hl) 								; get length into bits 0..5
		ld 		(ix+4),a 							; write as info/length byte
...
__dnwCopyName:
		...
		xor 	$20 								; actually stored in 7 bit ASCII because it makes
		add 	a,$20 								; debugging easier, you can read it in memory view.
...
DefaultCreateBits:									; default bits to set.
		db 		0
...
; ********************************************************************************************************
;
;							Set Default bits for new word
;
; ********************************************************************************************************

SetDefaultCreateBits:
		ld 		(DefaultCreateBits),a
		ret
