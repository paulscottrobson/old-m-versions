...
for f in sourceList:
	...
	for s in src:
		...
		if s.find("@desc") > 0:
			...
			f["doc"] = f["doc"].strip()
		if s.find("@info") > 0:
			if s.lower().find("compileonly"):
				compileOnlyList.append(f["word"])
	assert f["word"] != "" and f["type"] in ["macro","word","slow","fast"],"Bad type "+f["type"]+" "+f["word"]
...
h.close()
#
#		Output the coreonly.lst file
#
h = open("compileonly.lst","w")
h.write("__".join(compileOnlyList))
h.close()