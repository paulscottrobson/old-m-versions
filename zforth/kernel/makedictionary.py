...
#		Get the words
#
words = [decode(x) for x in open("kernel.lst").readlines() if x[:10] == "__zfdefine"]
...
for w in words:
	...
	image[p+4] = w[1] >> 8
	image[p+5] = 0x01 if w[2] == 'M' else 0x00						# type information.
	for c in range(0,len(w[0])):									# name
		image[p+6+c] = ord(w[0].upper()[c]) 
	image[p+6+len(w[0])] = 0										# ASCIIZ marker.
	p = p + len(w[0]) + 7
